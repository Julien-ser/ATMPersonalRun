...
class MITOModelEvaluator:
    def __init__(self, model_path: str, device: str = "cuda" if torch.cuda.is_available() else "cpu"):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(model_path, quantization_config=bnb_config, device_map="auto", torch_dtype=torch.float16)#.to(self.device)
        self.model.eval()
    ...
    def generate_response(self, input_text: str, max_length: int = 512) -> str:
        ...
        """
        """No device movement needed - model handles it automatically"""
        ...
        ).to(self.model.device)  # Critical: move to model's device
        ...
        with torch.no_grad():
            outputs = self.model.generate(
                ...
                temperature=0.7,
                top_p=0.9,
                use_cache=True,  # Actually saves memory with KV cache
