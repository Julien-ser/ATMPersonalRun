...
class MITOModelEvaluator:
    ...
    def generate_response(self, input_text: str, max_new_tokens: int = 16) -> str:
        """
        ...
        prompt_decoded = self.tokenizer.decode(inputs.input_ids[0], skip_special_tokens=False)
        answer = decoded_output[len(prompt_decoded):].strip()

        # Remove trailing prompt tokens if present
    ...
    def evaluate_single_example(self, example: Dict, max_tokens_per_chunk: int = 512, overlap: int = 1) -> Dict:
        ...
        # Chunk the context list to fit under model's input limit
        chunked_answers = []
        token_limit = max_tokens_per_chunk
        prompt_tokens = self.tokenizer(self.prepare_input(query, [{"text": ""}]), return_tensors="pt")['input_ids'].shape[1]

        # Estimate how many tokens we can fit per chunk
        max_context_tokens = token_limit - prompt_tokens - 64  # Leave room for generation
        ...
        for ctx in contexts:
            ctx_text = ctx["text"]
            ctx_tokens = self.tokenizer(ctx_text, return_tensors="pt")['input_ids'].shape[1]

            # If adding this context exceeds token limit, push current chunk
            if current_chunk_tokens + ctx_tokens > max_context_tokens:
                chunked_inputs.append(current_chunk)
                # Overlap by `overlap` items
                current_chunk = current_chunk[-overlap:] + [ctx]
                current_chunk_tokens = sum(
                    self.tokenizer(c["text"], return_tensors="pt")['input_ids'].shape[1] for c in current_chunk
                )
            else:
                current_chunk.append(ctx)
                current_chunk_tokens += ctx_tokens

        # Append remaining chunk
        if current_chunk:
            chunked_inputs.append(current_chunk)

        # Now run generation on each chunk
        best_answer = ""
        best_f1 = -1
        best_em = False
        best_subspan = False
        ...
        for chunk in chunked_inputs:
            input_text = self.prepare_input(query, chunk)
            answer = self.generate_response(input_text)

            f1 = compute_f1(answer, ground_truths)
            em = compute_exact_match(answer, ground_truths)
            subspan = compute_subspan_exact_match(answer, ground_truths)

            chunked_answers.append({
                "answer": answer,
                "f1_score": f1,
                "exact_match": em,
                "subspan_exact_match": subspan
            })
        ...
        return {
            ...
            "answer": best_answer,
            "ground_truth": ground_truths,
            "has_answer": bool(best_answer.strip()),
            "subspan_exact_match": best_subspan,
            "exact_match": best_em,
            "f1_score": best_f1,
...
if __name__ == "__main__":
    # Example usage
    import warnings
    base_model_path = "/home/julien/ATM-RAG/atm_train/mito/experiments/model_final"#"/home/julien/ATM-RAG/atm_train/generator_sft/experiments/model_final"
    ...
    with open("/home/julien/ATM-RAG/atm_train/mito/hotpot_style.jsonl") as f:
    ...
    evaluation_results = evaluator.evaluate_dataset(eval_data, output_file="mitoevaluation_results_chunks.json")
    
    print("\nEvaluation complete. Results saved to mitoevaluation_results_chunks.json")
