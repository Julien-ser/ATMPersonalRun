...
input_file = Path("datasets/wikitest.jsonl")  # Path to your JSONL input file
...
with open(input_file, 'r') as f:
    for i, line in tqdm(enumerate(f), desc="Processing examples", unit="example", total=500):
        ...
        answer = example["answers_objects"][0]["spans"][0]
        #print(answer)
        #exit()
        question = example["question_text"]
        qid = example["question_id"]
        supporting_facts = example["contexts"]
        #aliases = list(set(answers["aliases"] + answers["normalized_aliases"]))
        titles = [item["title"] for item in supporting_facts]#[example['context'][i][0] for i in range(len(example['context']))]
        text = [item["paragraph_text"] for item in supporting_facts]#["".join(example['context'][i][1]) for i in range(len(example['context']))]
        contexts = []
...
output_path = Path("datasets/wiki_test.jsonl")
